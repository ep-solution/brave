...
public class ITRetrofitRxJava2 extends ITHttp {
  ...
  interface Service {
    ...
    @GET("/") Maybe<ResponseBody> maybe();

    @GET("/") Observable<ResponseBody> observable();
  ...
  @Test public void createAsync_maybe_success() {
    rxjava_createAsync_success((service, observer) -> {
      try (CurrentTraceContext.Scope scope = currentTraceContext.newScope(context1)) {
  ...
  @Test public void createAsync_observable_success() {
    rxjava_createAsync_success((service, observer) -> {
      try (CurrentTraceContext.Scope scope = currentTraceContext.newScope(context1)) {
  ...
  @Test public void createAsync_single_success() {
    rxjava_createAsync_success((service, observer) -> {
      try (CurrentTraceContext.Scope scope = currentTraceContext.newScope(context1)) {
  ...
  private void rxjava_createAsync_success(BiConsumer<Service, TestObserver<Object>> subscriber) {
    ...
    assertThat(currentTraceContextObserver.onComplete)
        .isEqualTo(context1);
  }

  @Test public void createAsync_flowable_success() {
