...
public class HttpParser {
    ...
    /**
     * Override to change what data from the http request are parsed into the span representing it. By default, this
     * sets the span name to the http method and tags "http.method" and "http.path".
     * <p>
     * If you only want to change the span name, you can override {@link #spanName(HttpAdapter, Object)} instead.
     *
     ...
     * <p>
     * By default, this tags "http.status_code" when it is not 2xx. If there's an exception or the status code is
     * neither 2xx nor 3xx, it tags "error". This also overrides the span name based on the
     * {@link HttpAdapter#methodFromResponse(Object)} and {@link HttpAdapter#route(Object)} where possible (ex "get
     * /users/:userId").
     * <p>
     * If routing is supported, and a GET didn't match due to 404, the span name will be "get not_found". If it didn't
     * match due to redirect, the span name will be "get redirected". If routing is not supported, the span name is left
     * alone.
     * <p>
     * If you only want to change how exceptions are parsed, override {@link #error(Integer, Throwable, SpanCustomizer)}
     * instead.
     * <p>
    ...
    public <Resp> void response(HttpAdapter<?, Resp> adapter, @Nullable Resp res, @Nullable Throwable error,
    ...
    /**
     * Override to change what data from the http error are parsed into the span modeling it. By default, this tags
     * "error" as the exception or the status code if it is neither 2xx nor 3xx.
     * <p>
    ...
    protected void error(@Nullable Integer httpStatus, @Nullable Throwable error, SpanCustomizer customizer) {
        ...
        if (message != null) {
            ...
            customizer.tag("has_error", String.valueOf(true));
        }
    }
